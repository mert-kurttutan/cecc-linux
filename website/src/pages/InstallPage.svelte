...
<section class="py-2 text-center">
  <div class="mx-auto mb-6 max-w-3xl text-center">
    <h1 class="mx-auto mb-4 text-[clamp(2rem,4vw,3.6rem)] leading-none font-bold text-amber-700 dark:text-amber-400">{content.install.title}</h1>
    <p class="mx-auto max-w-3xl text-lg leading-relaxed text-slate-600 dark:text-slate-400">
      {content.install.intro}
    </p>
  </div>

  <div class="mx-auto grid w-full max-w-[900px] grid-cols-1 gap-4">
    {#each content.install.options as option}
      {#if option.command}
        <CommandCard
          title={option.title}
          body={option.body}
