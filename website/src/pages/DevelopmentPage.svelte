...
<section class="py-2 text-center">
  <div class="mx-auto mb-6 max-w-3xl text-center">
    <h1 class="mx-auto mb-4 text-[clamp(2rem,4vw,3.6rem)] leading-none font-bold text-amber-700 dark:text-amber-400">{content.development.title}</h1>
  ...
  <div class="mx-auto grid max-w-[900px] grid-cols-1 gap-4">
    {#each content.development.commands as group}
      {#if group.command}
        <CommandCard
          title={group.title}
